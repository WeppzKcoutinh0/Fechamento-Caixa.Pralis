...
export function calcularJanelaDatetime(
  data: string,
  horarioDe: string,
  horarioAte: string,
): JanelaDatetime {
...
export function extrairLinhas(
  fechamentos: FechamentoComItens[],
  tipo: TipoConsulta,
): LinhaConsulta[] {
  ...
  for (const f of fechamentos) {
    ...
    if (tipo === 'transferencias') {
      for (const t of f.transferenciasCaixa) {
        linhas.push({
          ...
          detalhe: [
            `${t.caixaOrigem || '?'} → ${t.caixaDestino || '?'}`,
            t.lacre ? `Lacre ${t.lacre}` : '',
          ]
            .filter(Boolean)
            .join(' · '),
    ...
    if (tipo === 'saidas') {
      for (const s of f.sangrias) {
        linhas.push({
          ...base,
          descricao: s.descricao || 'Sangria',
          detalhe: s.lacre ? `Lacre ${s.lacre}` : '',
