-- Atualiza salvar_fechamento pra gravar as 3 colunas novas de lancamentos (20260914120000).
-- Base: a versão FINAL da função (20260914100300, com todos os coalesce de default que o
-- jsonb_populate_record/extração de jsonb não aplica sozinho — bug já corrigido 3 vezes nesta
-- migração e quase reintroduzido aqui por engano, ao copiar a versão errada como ponto de
-- partida). Só a lista de colunas do insert de lancamentos muda.
...
begin
  ...
  if jsonb_array_length(coalesce(payload->'discriminacoes', '[]'::jsonb)) > 0 then
    ...
    select
      v_id, ord - 1, x->>'tipo', coalesce((x->>'qtd')::numeric, 0), coalesce(x->>'produto', ''),
