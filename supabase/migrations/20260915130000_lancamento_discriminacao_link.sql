-- Antes desta migration, a Identificação... não, a seção Lançamentos só permitia UM
-- lançamento visível por tipo (Despesa/Mercadoria/Retirada) na UI, então "discriminar itens"
-- filtrava só por `tipo` — isso bastava porque tipo já identificava o lançamento sozinho.
--
-- Decisão do usuário (15/09/2026): permitir N lançamentos por tipo, com botão "Adicionar
-- despesa/mercadoria/retirada" (mesmo padrão de Entradas/Sangrias, que já era uma lista).
-- Isso quebra a suposição "tipo = lançamento": agora é preciso saber a QUAL lançamento cada
-- item discriminado pertence. `lancamento_id` é essa ligação.
--
-- Nullable de propósito: nenhum dado real existe ainda neste app (ainda não foi publicado), mas
-- mesmo assim não forçamos not null aqui para não quebrar um fechamento salvo entre esta
-- migration e o deploy do client novo — item sem lancamento_id só não aparece em nenhum modal de
-- edição até o usuário reabrir e salvar de novo.

alter table public.discriminacoes
  add column lancamento_id uuid references public.lancamentos (id) on delete cascade;

create index discriminacoes_lancamento_id_idx on public.discriminacoes (lancamento_id);

-- salvar_fechamento: grava o `id` do lançamento (gerado no client) em vez de deixar o default
-- do banco escolher — é isso que permite ao client já saber o id ANTES de salvar, pra ligar a
-- discriminação a ele no mesmo payload. E grava `lancamento_id` em cada discriminação.
