-- "Transferência entre caixas" — versão simplificada (decisão do usuário, 15/09/2026) da
-- Transferência do Sistema Inteligente Pralís: lá é um sistema de contas com múltiplos destinos
-- candidatos e confirmação por outra pessoa (ver PROJECT_BRAIN/14 daquele projeto); aqui é só um
-- registro de que dinheiro mudou de um caixa para outro, dentro do próprio fechamento — sem
-- cadastro de contas, sem confirmação, sem prazo.
--
-- Puramente informativo: não entra no cálculo de `diferenca`/`valor_total_final` (ver
-- app/utils/financeiro.ts) — sem confirmação do outro lado, não há garantia de que o valor
-- realmente chegou.

create table public.transferencias_caixa (
  id uuid primary key default gen_random_uuid(),
  fechamento_id uuid not null references public.fechamentos (id) on delete cascade,
  ordem integer not null default 0,
  caixa_origem text check (caixa_origem in ('Caixa 1', 'Caixa 2', 'Caixa 3', 'Caixa 4')),
  caixa_destino text check (caixa_destino in ('Caixa 1', 'Caixa 2', 'Caixa 3', 'Caixa 4')),
...
create index transferencias_caixa_fechamento_id_idx on public.transferencias_caixa (fechamento_id, ordem);

alter table public.transferencias_caixa enable row level security;

create policy "transferencias_caixa_authenticated_all" on public.transferencias_caixa
  for all
  to authenticated
  using (true)
  with check (true);

-- salvar_fechamento: acrescenta o delete+insert de transferencias_caixa, mesmo padrão dos outros
-- filhos. Nenhuma outra parte da função muda.
