-- "Relatório PDV" vira uma LISTA de entradas (pedido do usuário, 15/09/2026) — antes só existia
-- um único PDV por fechamento (colunas nr_clientes/pdv_dinheiro/... direto em `fechamentos`,
-- mesmo padrão que Despesas/Mercadorias/Retiradas tinham antes de virarem lista). Segue o MESMO
-- padrão relacional já usado pros outros filhos (entradas, sangrias, lancamentos): tabela própria,
-- RLS permissiva pra authenticated, delete+insert atômico dentro de salvar_fechamento.
--
-- `relatorio_pdv` (texto livre) e `img_pdv_path` (imagem) continuam ÚNICOS por fechamento — não
-- fazem sentido por PDV individual, são notas/anexo do fechamento como um todo.
-- `total_pdv`/`ticket_medio` continuam em `fechamentos` (somados no client a partir da lista antes
-- de salvar, mesmo padrão de `valor_total_final`/`diferenca` — a RPC nunca recalcula, só persiste).

create table public.pdv_entradas (
...
create index pdv_entradas_fechamento_id_idx on public.pdv_entradas (fechamento_id, ordem);

alter table public.pdv_entradas enable row level security;

create policy "pdv_entradas_authenticated_all" on public.pdv_entradas
  for all
  to authenticated
  using (true)
  with check (true);

-- Colunas antigas (um único PDV por fechamento) removidas — nenhum fechamento real foi salvo
-- ainda neste projeto (confirmado antes desta migration), então não há dado pra migrar.
alter table public.fechamentos
  drop column nr_clientes,
  drop column pdv_dinheiro,
  drop column pdv_credito,
  drop column pdv_debito,
  drop column pdv_pix,
  drop column pdv_voucher,
  drop column pdv_crediario;

-- salvar_fechamento: remove os campos de PDV único (não existem mais em `fechamentos`) e
-- acrescenta o delete+insert de pdv_entradas, mesmo padrão dos outros filhos.
...
begin
  ...
  if jsonb_array_length(coalesce(payload->'lancamentos', '[]'::jsonb)) > 0 then
    insert into public.lancamentos (
      ...
      vencimento, data_pagamento, foto_nota_path
    )
    select
      ...
      nullif(x->>'dataPagamento','')::date, x->>'fotoNotaPath'
