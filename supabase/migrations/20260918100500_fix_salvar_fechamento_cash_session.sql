-- Atualiza salvar_fechamento pra ida-e-volta de `cash_session_id` (20260918100300). Base: a
-- versão de 20260916190100 (origem_ajuste_creare) — só a linha do coalesce-preserve e o `set`
-- do upsert mudam.
--
-- `v_row.cash_session_id := coalesce(v_row.cash_session_id, (select ... from fechamentos))`
-- ANTES do insert: mesma proteção que `criado_por` já tem hoje (nunca é sobrescrito no `on
-- conflict do update set`, porque simplesmente não está na lista) — aqui é preciso ser
-- EXPLÍCITO porque `cash_session_id` PRECISA estar na lista do `set` (senão uma re-gravação nunca
-- gravaria o vínculo a sessão nenhuma vez), então sem esse coalesce um admin editando o
-- fechamento de um caixa (sem o payload carregar `cash_session_id`, já que o tipo TypeScript só
-- foi estendido nesta mesma revisão) apagaria o vínculo com a sessão original.
