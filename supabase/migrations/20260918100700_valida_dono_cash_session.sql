-- Achado real do agente de pentest (18/09/2026, rodada final de validação do Fluxo de Caixa):
-- `salvar_fechamento` deixava um usuário CAIXA criar um fechamento SEU (`criado_por` continua
-- forçado ao próprio uid, RLS de insert em `fechamentos` continua passando) mas com
-- `cash_session_id` apontando pra sessão de OUTRO usuário, sem nenhuma checagem de dono. Não
-- vazava/alterava dado alheio (RLS em fechamentos/filhos e em `cash_sessions.fechamento_id`
-- continuam travadas por `criado_por`/`opened_by`, confirmado no pentest), mas criava um
-- fechamento forjado plausível "pendurado" na sessão de outra pessoa — qualquer relatório futuro
-- que junte `cash_sessions -> fechamentos` por `cash_session_id` mostraria isso como se fosse do
-- dono da sessão.
--
-- Correção: mesmo princípio de `fechamento_editavel`/`fechamento_visivel` (20260918100400) —
-- `security invoker` já dá `auth.uid()` correto aqui dentro. Se `cash_session_id` vier
-- preenchido (novo ou vindo do coalesce-preserve), tem que pertencer (`opened_by = auth.uid()`)
-- a quem está chamando, a menos que seja admin (admin pode religar/corrigir o vínculo de
-- qualquer fechamento, mesmo scenario de "admin edita fechamento de um caixa" que o
-- coalesce-preserve já cobre).
