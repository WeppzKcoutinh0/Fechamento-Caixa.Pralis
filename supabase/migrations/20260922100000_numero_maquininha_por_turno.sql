-- Cada turno pode usar uma maquininha diferente. Mantemos `nr_maquininha` como o número
-- da manhã para não perder os fechamentos já existentes e adicionamos o número da tarde.
alter table public.fechamentos
  add column if not exists nr_maquininha_tarde text not null default '';

-- Reaplica a última versão da RPC, acrescentando somente o novo campo no preenchimento e no
-- update. O restante do fluxo continua idêntico ao da migration 20260921090000.
