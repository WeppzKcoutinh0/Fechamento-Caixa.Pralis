-- Vendas/Produtos Cancelados (Relatório Final, pedido do usuário 23/09/2026) — cenário pronto pra
-- quando o robô de vendas (CREARE/Compliart) for adaptado pra também enviar vendas canceladas
-- (hoje só manda vendas com STATUS='F', ver integracoes-scripts/queries/FECHAMENTO_CAIXA.sql).
-- Por enquanto só o motivo (texto ou áudio) é dado real — a lista de itens cancelados em si fica
-- vazia até o robô estar pronto (ver app/composables/useVendasCanceladas.ts).
alter table public.fechamentos
  add column if not exists vendas_canceladas_motivo_tipo text not null default 'texto'
    check (vendas_canceladas_motivo_tipo in ('texto', 'audio')),
  add column if not exists vendas_canceladas_motivo_texto text not null default '',
  add column if not exists vendas_canceladas_motivo_audio_path text;
